...
// Auto-create room_requests table if it doesn't exist
$db->exec("
...
// Student + room
$sq = $db->prepare("SELECT s.room_id, s.full_name, r.room_number FROM students s LEFT JOIN rooms r ON r.room_id = s.room_id WHERE s.student_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($chk->fetchColumn() > 0) {
        $error = "You already have a pending room change request. Please wait for a response before submitting another.";
    } else {
        ...
        if (empty($reason)) {
            $error = "Please provide a reason for your room change request.";
        } else {
            $ins = $db->prepare("
            ...
            $success = "Your room change request has been submitted. The hostel office will get back to you.";
        }
    }
}
...
<div class="page">
    ...
    <div class="page-title">
        ...
        <p>Submit a request to the hostel office to change your room.</p>
    </div>
            ...
            <div class="card-body">
                ...
                <form method="POST" action="room_request.php">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="reason" placeholder="Please explain why you'd like to change your room…" required></textarea>
                        <p class="form-hint">Be as specific as possible — this helps the admin process your request faster.</p>
                    </div>
            ...
            <div class="card-body">
                <ul class="info-list">
                    <li style="flex-direction:column;align-items:flex-start;gap:4px">
                        ...
                        <span style="color:#6b7280;font-size:0.82rem">Fill in the form with your preferred room type, floor, and the reason.</span>
                    </li>
                    <li style="flex-direction:column;align-items:flex-start;gap:4px">
                        <span style="font-weight:700;color:#1a1a2e">2. Admin reviews it</span>
                        <span style="color:#6b7280;font-size:0.82rem">The hostel office checks availability and reviews your request.</span>
                    </li>
                    <li style="flex-direction:column;align-items:flex-start;gap:4px">
                        <span style="font-weight:700;color:#1a1a2e">3. You get a response</span>
                        <span style="color:#6b7280;font-size:0.82rem">The status below will be updated to Approved or Rejected with a note.</span>
                    </li>
                </ul>
                <div style="margin-top:14px;font-size:0.8rem;color:#9ca3af">
                    You can only have <strong>one pending request</strong> at a time.
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="tbl-wrap">
            <table>
                <thead>
                    <tr><th>Submitted</th><th>Current Room</th><th>Preferred Type</th><th>Preferred Floor</th><th>Reason</th><th>Status</th><th>Admin Note</th></tr>
                </thead>
                <tbody>
                <?php foreach ($requests as $r): ?>
                <tr>
                    <td><?= date('d M Y', strtotime($r['created_at'])) ?></td>
                    <td><?= $r['current_room_id'] ? '#'.$r['current_room_id'] : '—' ?></td>
