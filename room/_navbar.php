<?php
// ─────────────────────────────────────────────────────────────────────────────
// room/_navbar.php  –  Shared Navigation Bar for the Room Module
//
// This file is included at the top of every page in the room/ folder.
// It draws the top navigation bar with links to all admin modules.
//
// HOW TO USE:
//   Set $activeNav to a module name BEFORE including this file.
//   That name gets compared below to add the 'active' CSS class to the right link.
//
//   Example:
//       $activeNav = 'rooms';
//       include '_navbar.php';
// ─────────────────────────────────────────────────────────────────────────────

// Default to 'rooms' if the page forgot to set $activeNav.
$activeNav = $activeNav ?? 'rooms';
?>
<nav class="navbar">

    <!-- ── Left side: Brand icon + app name ── -->
    <div class="navbar-left">
        <!-- House SVG icon (decorative) -->
        <div class="brand-icon">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                <path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>
                <polyline points="9 22 9 12 15 12 15 22"/>
            </svg>
        </div>
        <!-- Application name and tagline -->
        <div class="brand-text">
            <h1>HostelHub</h1>
            <small>MANAGEMENT SYSTEM</small>
        </div>
    </div>

    <!-- ── Centre: Navigation links to every module ── -->
    <!-- The 'active' class is applied to whichever link matches $activeNav,
         making it visually highlighted in the navbar. -->
    <div class="nav-links">
        <a href="../dashboard.php"               class="<?php echo $activeNav==='dashboard'   ? 'active' : ''; ?>">Dashboard</a>
        <a href="../Student module/index.php"    class="<?php echo $activeNav==='students'    ? 'active' : ''; ?>">Students</a>
        <a href="index.php"                      class="<?php echo $activeNav==='rooms'       ? 'active' : ''; ?>">Rooms</a>
        <a href="../fees/index.php"              class="<?php echo $activeNav==='fees'        ? 'active' : ''; ?>">Fees</a>
        <a href="../maintenance/index.php"       class="<?php echo $activeNav==='maintenance' ? 'active' : ''; ?>">Maintenance</a>
        <a href="../users/index.php"             class="<?php echo $activeNav==='users'       ? 'active' : ''; ?>">Users</a>
    </div>

    <!-- ── Right side: Logged-in user info + logout button ── -->
    <div class="navbar-right">
        <div class="user-info">
            <!-- Show the logged-in admin's username from the session.
                 Fallback text shown if the session variable is somehow missing. -->
            <div class="name"><?php echo isset($_SESSION['username']) ? htmlspecialchars($_SESSION['username']) : 'System Administrator'; ?></div>
            <div class="role">Admin</div>
        </div>
        <!-- Logout link — goes to logout.php which clears the session -->
        <a href="logout.php" class="logout-btn">Logout</a>
    </div>

</nav>
