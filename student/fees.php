...
// Summary
$sq = $db->prepare("SELECT COALESCE(SUM(amount),0) AS total, COALESCE(SUM(CASE WHEN is_paid IS NOT NULL THEN amount ELSE 0 END),0) AS paid, COALESCE(SUM(CASE WHEN is_paid IS NULL THEN amount ELSE 0 END),0) AS unpaid, COALESCE(SUM(CASE WHEN is_paid IS NULL AND due_date < CURDATE() THEN amount ELSE 0 END),0) AS overdue FROM fees WHERE student_id = ?");
$sq->execute([$sid]); $sum = $sq->fetch();
...
<div class="page">
    ...
    <div class="page-title">
        <h1>My Fees</h1>
        <p>All fee records associated with your account</p>
    </div>
