...
if (isset($_POST['mark_paid'])) {
    ...
    $db->prepare("
        UPDATE fees SET is_paid=1, paid_at=NOW(), payment_method=?
...
if (isset($_POST['update'])) {
    $stmt = $db->prepare("
        UPDATE fees SET
            student_id=?, fee_type=?, amount=?, due_date=?, payment_method=?
        WHERE receipt_number=?
    ");
    $stmt->execute([
        $_POST['student_id'], $_POST['fee_type'],
        $_POST['amount'], $_POST['due_date'],
        $_POST['payment_method'], $id
    ]);
    header("Location: index.php");
    exit;
}
...
<form method="POST">
    ...
    <button type="submit" name="mark_paid" style="background:#28a745;margin-bottom:10px;">
        Mark as Paid
    </button>
</form>
...
<form method="POST">
    <label>Receipt</label>
    <input type="text" value="<?= htmlspecialchars($fee['receipt_number']) ?>" readonly>
...
<br>
<a href="index.php" style="display:block;text-align:center;color:#555;">&#8592; Back to Dashboard</a>
</div>
</div>
</div>
</body>
